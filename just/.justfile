...
update-offline-bins:
    ...
    tmpdir=$(mktemp -d) && pushd $tmpdir \
        && scuba --image rust:1.84-bullseye cargo install harper-ls --locked --root ./offline \
    ...
    tmpdir=$(mktemp -d) && pushd $tmpdir \
        ...
        && cd helix \
        && scuba --image rust:1.84-bullseye cargo build --release -p helix-term --target x86_64-unknown-linux-gnu \
    ...
    curl -svL https://static.rust-lang.org/dist/rust-1.84.0-x86_64-unknown-linux-musl.tar.xz -o ~/offline/rust-1.84.0-x86_64-unknown-linux-musl.tar.xz
    curl -svL https://static.rust-lang.org/dist/rust-1.84.0-x86_64-unknown-linux-gnu.tar.xz -o ~/offline/rust-1.84.0-x86_64-unknown-linux-gnu.tar.xz
